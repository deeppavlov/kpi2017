...
class POSTagger(nn.Module):
    ...
    def gpu(self, obj):
        if self.opt.get('cuda') and torch.cuda.is_available():
            return obj.cuda(self.opt.get('gpu'))
        else:
            return obj
    ...
    def __init__(self, opt, word_dict, state_dict=None):
        ...
        self.optimizer = torch.optim.Adam(self.parameters(), self.opt['learning_rate'])
        if state_dict:
