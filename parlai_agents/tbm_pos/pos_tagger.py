...
class POSTagger(nn.Module):
    ...
    def __init__(self, opt, word_dict, state_dict=None, word_embeddings_size=50, word_window_size=2,
                 pos_embedding_size=15, prev_pos_count=3, ngram_embedding_info=((500, 16), (200, 16), (4000, 16))):
        ...
        self.ngram_embedding_info = ngram_embedding_info
        ...
        self.char_gram_embs = [self.gpu(nn.Embedding(count, size)) for count, size in ngram_embedding_info]
        ...
        self.linear = self.gpu(nn.Linear(self.word_embedding_size*words_count +
                                         ...
                                         sum([ei[1] for ei in ngram_embedding_info]),
    ...
    def word_char_emb(self, str_word):
        ...
        wl = len(w)
        embeddings = []
        for n, (ec, es), emb in zip([2, 3, 4], self.ngram_embedding_info, self.char_gram_embs):
            emb_ids = []
            for i in range(wl - n + 1):
                emb_ids.append(hash(w[i:i+n+1]) % ec)
            if emb_ids:
    ...
    def set_input(self, input_seq):
        ...
        str_words = ['']*self.word_windows_size + list(self.word_dict.tokenize(input_seq)) + ['']*self.word_windows_size
        char_embs = torch.cat([self.word_char_emb(w) for w in str_words if w], dim=0)
        ...
        return POS_Tagger_State(words, char_embs, 0, words_count, start, None,
                                words_count == 0)
    ...
    def act(self, state=None, action=None, output=None):
        ...
        return POS_Tagger_State(state.words, state.char_embs, state.input_index + 1, state.words_count, (state.prev_pos + [action])[-self.prev_pos_count:], output, terminated)
    ...
    def embed_states(self, states):
        ...
        for state in states:
            ...
            chars.append(state.char_embs[state.input_index])
        word_embeddings = torch.cat(words, dim=0)
        prev_pos = self.gpu(Variable(torch.LongTensor(prev_pos)))
        return word_embeddings, self.pos_emb(prev_pos).view(len(states), -1), torch.stack(chars)

    def forward(self, states):
        word_embeddings, pos_embeddings, char_embeddings = self.embed_states(states)
        x = torch.cat([word_embeddings, pos_embeddings, char_embeddings], dim=1)
